...
//属性とスキルレベルと座標を与えてパーティクルを生成する関数
  ...
  　　//HealとSlipスキル追加
/*
    var skillName = ["YamiTexture"];

    for(var i = 1; i < 3; i++){
      var sName = "res." + skillName[attrib] + i + "_plist";

      var tempParticle = new cc.ParticleSystem(eval(sName));
      tempParticle.setLife(100);
      tempParticle.setPosition(x + 340, y + 100);
...
*/
var skillName = ["YamiTexture"];
    var sName = "res." + skillName[attrib]  + rare + "_plist";

    debugText.setString(sName);

    var tempParticle = new cc.ParticleSystem(eval(sName));
    tempParticle.setSpeed(6.5);
    tempParticle.setLife(6.5);
    tempParticle.setPosition(x + 340, y + 100);
    this.addChild(tempParticle, 20);
    tempParticle.setAutoRemoveOnFinish(true);
